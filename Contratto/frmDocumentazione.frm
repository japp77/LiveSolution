...
Begin VB.Form frmDocumentazione 
   ...
   EndProperty
   Icon            =   "frmDocumentazione.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
    ...
    Case 3 'VISUALIZZATO DAL CONTRATTO
        Select Case IDStartUpFunzione
            ...
            Case 3 'Pulsante contratto
                GET_SQL = "WHERE IDAzienda=" & LINK_AZIENDA_DOC
                GET_SQL = GET_SQL & " AND IDAnagrafica=" & LINK_CLIENTE_DOC
                GET_SQL = GET_SQL & " AND IDRV_POContrattoPadre=" & LINK_CONTRATTO_DOC_PADRE
                If Generale = 1 Then
                    GET_SQL = GET_SQL & " AND IDRV_POInterventoPadre=0"
                    GET_SQL = GET_SQL & " AND IDRV_POIntervento=0"
                End If
...
sSQL = sSQL & sSQL_WHERE
    ...
    With Me.GrigliaDocumentazione
        ...
        .ColumnsHeader.Clear
            ...
            .ColumnsHeader.Add "IDRV_POContrattoPadre", "IDRV_POContrattoPadre", dgNumeric, False, 500, dgAlignleft, True, True, False
            ...
            .ColumnsHeader.Add "AnnoContratto", "Anno Contratto", dgInteger, True, 1000, dgAlignRight, True, True, False
            .ColumnsHeader.Add "NumeroContratto", "N° Contratto", dgInteger, True, 1000, dgAlignRight, True, True, False
            .ColumnsHeader.Add "NumeroRinnovo", "N° Rinnovo", dgInteger, True, 1000, dgAlignRight, True, True, False
...
Dim PercorsoFile As String
    ...
    'For I = 1 To Data.Files.Count
        rsNew.AddNew
            ...
            rsNew!IDRV_POContrattoPadre = LINK_CONTRATTO_DOC_PADRE
...
On Error GoTo ERR_cmdPercorso_Click
...
AVVIA_FILE IDDocumentazione, sDir & "\", NomeFile, fnNotNullN(Me.GrigliaDocumentazione.AllColumns("Archiviato").Value)

Exit Sub
ERR_cmdPercorso_Click:
MsgBox Err.Description, vbCritical, "ERR_cmdPercorso_Click"
...
'txtPercorso.Text = Data.Files.Item(1)

'MsgBox Data.Files.Item(1)

'Set F = New FileSystemObject

'If F.FileExists(txtPercorso.Text) = True Then
'    Set NFile = F.GetFile(txtPercorso.Text)
'    Me.txtNomeFile.Text = NFile.Name
'    Set NFile = Nothing
'End If
...
Private Sub GET_CONTROLLO_PULSANTI()
    ...
    End Select
End Sub
Private Sub ELIMINAZIONE_DOCUMENTAZIONE()
On Error GoTo ERR_ELIMINAZIONE_DOCUMENTAZIONE
Dim Testo As String
Dim sSQL As String
Dim IDDocumentazione As Long

    If ((Me.GrigliaDocumentazione.Recordset.EOF = True) And (Me.GrigliaDocumentazione.Recordset.BOF = True)) Then Exit Sub
    
    Testo = "Sei sicuro di voler eliminare il documento?"
    
    If MsgBox(Testo, vbQuestion + vbYesNo, "Eliminazione documentazione") = vbNo Then Exit Sub
    
    IDDocumentazione = fnNotNullN(Me.GrigliaDocumentazione.AllColumns("IDRV_PODocumentazione").Value)
        
    'eliminazione documentazione'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Exit Sub
ERR_ELIMINAZIONE_DOCUMENTAZIONE:
    MsgBox Err.Description, vbCritical, "Eliminazione documentazione"
End Sub

Private Sub GrigliaDocumentazione_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyDelete Then
        ELIMINAZIONE_DOCUMENTAZIONE
    End If
End Sub

Private Sub GrigliaDocumentazione_OLEDragDrop(Data As DataObject, Effect As Long, Button As Integer, Shift As Integer, X As Single, Y As Single)
MsgBox Data.Files.Item(1)
End Sub
...
For I = 1 To Data.Files.Count
    rsNew.AddNew
        ...
        rsNew!IDRV_POInterventoPadre = LINK_INTERVENTO_PADRE_DOC
        rsNew!IDRV_POIntervento = LINK_INTERVENTO_DOC
        rsNew!PercorsoOriginale = Data.Files.Item(I)
        rsNew!PercorsoCompleto = Data.Files.Item(I)
        rsNew!NomeFile = GET_NOME_FILE(Data.Files.Item(I))
        rsNew!TipoFile = GetExt(rsNew!NomeFile)
        rsNew!Annotazioni = ""
        rsNew!Archiviato = 1
        rsNew!IDUtenteInserimento = LINK_UTENTE
        ...
        If Abs(rsNew!Archiviato) = 1 Then
            ...
            sts.Type = ADODB.adTypeBinary
            sts.Open
            sts.LoadFromFile Data.Files.Item(I)
            rsNew!Documento = sts.Read
        End If
    rsNew.Update
Next
