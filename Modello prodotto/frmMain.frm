      ...
      Begin VB.PictureBox PicForm 
         ...
         Begin VB.PictureBox PicForm2 
            ...
            Begin VB.Frame Frame1 
               ...
               Top             =   0
               Width           =   8655
               Begin DMTDataCmb.DMTCombo cboMarcaProdotto 
                  Height          =   315
                  Left            =   5280
                  TabIndex        =   1
                  Top             =   600
                  ...
                  EndProperty
               End
               Begin VB.TextBox txtModelloProdotto 
                  Height          =   315
                  Left            =   120
                  TabIndex        =   0
                  Top             =   600
...
Private Const CAMPO_PER_CAPTION = "ModelloProdotto"
...
Private Function PermissionToSave() As Boolean
    ...
    If Len(Trim(Me.txtModelloProdotto.Text)) = 0 Then
        MsgBox "Inserire il modello", vbCritical, "Controllo dati"
        Exit Function
    End If
    
    If Me.cboMarcaProdotto.CurrentID = 0 Then
        MsgBox "Inserire la marca", vbCritical, "Controllo dati"
        Exit Function
    End If
    
    If GET_ESISTENZA_MODELLO = True Then
        MsgBox "Configurazione esistente", vbCritical, "Controllo dati"
        Exit Function
    End If
...
Private Sub CreateFormFields()
    ...
    Set Field.Control = Me.txtModelloProdotto
    Field.Name = "ModelloProdotto"
    Field.Visible = True
    Me.txtModelloProdotto.Tag = Field.Name
    m_FormFields.Add Field
    
    'IDRV_POMarcaProdotto
    Set Field = New FormField
    Set Field.Control = Me.cboMarcaProdotto
    Field.Name = "IDRV_POMarcaProdotto"
    Field.Visible = True
    Me.cboMarcaProdotto.Tag = Field.Name
    m_FormFields.Add Field
End Sub
...
Private Sub CreateBrowserConditions()
    ...
    Set Cond = BrwMain.Conditions.Add("ModelloProdotto", "Modello prodotto", m_DocType.TableName, False, False, False, dgCondTypeText)
    Set Cond = BrwMain.Conditions.Add("IDRV_POMarcaProdotto", "Marca prodotto", m_DocType.TableName, False, False, , dgCondTypeComboDB)
...
Private Sub txtModelloProdotto_Change()
 If Not (BrwMain.Visible) Then Change
End Sub
Private Function GET_ESISTENZA_MODELLO() As Boolean
Dim sSQL As String
Dim rs As DmtOleDbLib.adoResultset

sSQL = "SELECT IDRV_POModelloProdotto FROM RV_POModelloProdotto "
sSQL = sSQL & " WHERE IDRV_POMarcaProdotto=" & Me.cboMarcaProdotto.CurrentID
sSQL = sSQL & " AND ModelloProdotto=" & fnNormString(Me.txtModelloProdotto.Text)
sSQL = sSQL & " AND IDRV_POModelloProdotto<>" & fnNotNullN(m_Document(m_Document.PrimaryKey).Value)

Set rs = Cn.OpenResultset(sSQL)

If rs.EOF Then
    GET_ESISTENZA_MODELLO = False
Else
    GET_ESISTENZA_MODELLO = True
End If
