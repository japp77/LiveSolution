...
Private Const CAMPO_PER_CAPTION = "Stagione"
...
Private Function PermissionToSave() As Boolean
    ...
    If Len(Trim(Me.txtStagione.Text)) = 0 Then
        MsgBox "Inserire una descrizione della stagione", vbCritical, "Salvataggio dati"
        Me.txtStagione.SetFocus
        Exit Function
    End If
    
    If Me.txtDataInizio.Value = 0 Then
        MsgBox "Inserire una data di inizio della stagione", vbCritical, "Salvataggio dati"
        Me.txtDataInizio.SetFocus
        Exit Function
    End If
    
    If Me.txtDataFine.Value = 0 Then
        MsgBox "Inserire una data di fine della stagione", vbCritical, "Salvataggio dati"
        Me.txtDataFine.SetFocus
        Exit Function
    End If
    
    If Me.txtDataInizio.Value > Me.txtDataFine.Value Then
        MsgBox "Le date di inzio e fine stagione non sono congruenti", vbCritical, "Salvataggio dati"
        Me.txtDataFine.SetFocus
        Exit Function
...
Private Sub CreateFormFields()
    ...
    Set Field.Control = Me.txtStagione
    Field.Name = "Stagione"
    Field.Visible = True
    Me.txtStagione.Tag = Field.Name
    m_FormFields.Add Field
        
    'Data inizio
    Set Field = New FormField
    Set Field.Control = Me.txtDataInizio
    Field.Name = "DataInizio"
    Field.Visible = True
    Me.txtDataInizio.Tag = Field.Name
    m_FormFields.Add Field

    'Data Fine
    Set Field = New FormField
    Set Field.Control = Me.txtDataFine
    Field.Name = "DataFine"
    Field.Visible = True
    Me.txtDataFine.Tag = Field.Name
    m_FormFields.Add Field
...
Private Sub CreateBrowserConditions()
    ...
    Set Cond = BrwMain.Conditions.Add("Stagione", "Stagione", m_DocType.TableName, False, False, False, dgCondTypeText)
    Set Cond = BrwMain.Conditions.Add("DataInizio", "Data inizio", m_DocType.TableName, False, True, False, dgCondTypeDate)
    Set Cond = BrwMain.Conditions.Add("DataFine", "Data fine", m_DocType.TableName, False, True, False, dgCondTypeDate)
...
Private Sub txtStagione_Change()
    If Not (BrwMain.Visible) Then Change
End Sub
